...
1. In the [Azure Management Portal](https://portal.azure.com), go to the Azure Data Lake Analytics (ADLA) service that you created, and select **Overview** > **New Job**.
...
### Transform MAG data into MAKES entities json
...
1. Copy and paste the following code block in the script window.
    ...
    DECLARE EXTERNAL @Out_OutputPath string = "";  // Example: "wasb://subgraphs@mymagstore/mag-prod-2020-02-07/microsoft"
    
    // Output files by entity type. Used for debug purposes. Dsiabled by default.
    ...
    ////////////////////////////////////////////////////////////////////////////////////////////////////
    // Controls whether to output entities files by entity type
    ...
    IF !string.IsNullOrEmpty(@Param_UseSubgraphForInstitution) THEN
        ...
        //You can create subgraphs based on other constraints by modifying the @papers table. This particular script will
...
1. Replace placeholder values in the script using the table below
   ...
   |**`@Out_OutputPath string`** | The entity output wasb path to where you'd like the formatted entities documents to go. | **wasb://makessubgraph@mymakesstore/2020-02-07/microsoft/entities**  |
...
1. Replace the command parameters with the values from the table below:
    ...
    |---------|---------|------|
    |**`<IndexResourceName>`** | The name of the indexing resources you created above. | **makesindexres** |
    |**`<MakesPackageLocation>`** | The base URL to a MAKES release. |  **https://mymakesstore.blob.core.windows.net/makes/2020-02-07/**|
    |**`<MakesIndexResourceConfigFilePath>`** | The file path and name of the file that will hold the configuration information to build your index. | **makesIndexResConfig.json** |

### Submit a build index job
...
You can check the build index job progress using Azure Management Portal. Each job should start with a jobManager task, then preprocessor tasks, and finally indexer tasks. The number of preprocessor tasks should match the number of input entities file. The number of indexer tasks should match the **--IndexPartitionCount** parameter used for the BuildIndex command.
