...
1.  Provide following values to create an Azure Storage account. Then click "Review + create" button.
    ...
    |**Resource group** | Specify whether you want to create a new resource group or use an existing one. A resource group is a container that holds related resources for an Azure solution. For more information, see [Azure Resource Group overview](../../azure/azure-resource-manager/resource-group-overview.md). |
